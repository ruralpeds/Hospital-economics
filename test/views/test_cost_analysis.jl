# ============================================================================
# Unit tests for app/views/cost_analysis/cost_analysis.jl (E7)
# ============================================================================
...
@testset "CostAnalysis view" begin
    model = FakeModel()

    @testset "ui_cost_analysis returns non-empty result" begin
        result = ui_cost_analysis(model)
        @test result isa Dict
        @test !isempty(result)
    end

    @testset "ui_cost_analysis uses app_layout" begin
        result = ui_cost_analysis(model)
        @test result[:fn] == :app_layout
    end

    @testset "title is Cost Analysis" begin
        result = ui_cost_analysis(model)
        @test result[:title] == "Cost Analysis"
    end
