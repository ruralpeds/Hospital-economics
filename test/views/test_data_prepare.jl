# ============================================================================
# Unit tests for app/views/data_prepare/data_prepare.jl (E5)
# ============================================================================
...
@testset "DataPrepare view" begin
    model = FakeModel()

    @testset "ui_data_prepare returns non-empty result" begin
        result = ui_data_prepare(model)
        @test result isa Dict
        @test !isempty(result)
    end

    @testset "ui_data_prepare uses app_layout" begin
        result = ui_data_prepare(model)
        @test result[:fn] == :app_layout
    end

    @testset "title is Data Preparation" begin
        result = ui_data_prepare(model)
        @test result[:title] == "Data Preparation"
    end
