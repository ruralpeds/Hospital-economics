# ============================================================================
# Unit tests for app/views/cohorts/cohorts.jl (E6)
# ============================================================================
...
@testset "Cohorts view" begin
    model = FakeModel()

    @testset "ui_cohorts returns non-empty result" begin
        result = ui_cohorts(model)
        @test result isa Dict
        @test !isempty(result)
    end

    @testset "ui_cohorts uses app_layout" begin
        result = ui_cohorts(model)
        @test result[:fn] == :app_layout
    end

    @testset "title is Cohort Builder" begin
        result = ui_cohorts(model)
        @test result[:title] == "Cohort Builder"
    end
