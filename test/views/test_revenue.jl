# ============================================================================
# Unit tests for app/views/revenue/revenue.jl (E8)
# ============================================================================
...
@testset "Revenue view" begin
    model = FakeModel()

    @testset "ui_revenue returns non-empty result" begin
        result = ui_revenue(model)
        @test result isa Dict
        @test !isempty(result)
    end

    @testset "ui_revenue uses app_layout" begin
        result = ui_revenue(model)
        @test result[:fn] == :app_layout
    end

    @testset "title is Revenue & Reimbursement" begin
        result = ui_revenue(model)
        @test result[:title] == "Revenue & Reimbursement"
    end
